    ...
    try:
        if side == "src":
            url = (f"mysql+pymysql://{quote_plus(data['src_user'])}:"
                   ...
                   f"{data['src_port']}/{data['src_db']}?charset=utf8mb4")
            label = "源库"
...
def table_preview_data(conn_data, database, table_name, schema='', order_col='', order_dir=''):
    ...
    try:
        ...
        if db_type == 'oracle':
            if order_clause:
                limit_sql = f"SELECT * FROM (SELECT * FROM {tbl}{order_clause}) WHERE ROWNUM <= 50"
            else:
                limit_sql = f"SELECT * FROM {tbl} WHERE ROWNUM <= 50"
        elif db_type == 'mssql':
            limit_sql = f"SELECT TOP 50 * FROM {tbl}{order_clause}"
        else:
            limit_sql = f"SELECT * FROM {tbl}{order_clause} LIMIT 50"
...
def table_save_changes(conn_data, database, table_name, schema, changes):
    ...
    try:
        ...
        for ch in changes:
            col = ch.get("col", "")
            new_val = ch.get("newVal", "")
            if col not in col_vals:
                col_vals[col] = set()
            if new_val not in col_vals[col]:
                col_vals[col].add(new_val)
                actual_changes.append(ch)

        sqls = []
        for ch in actual_changes:
            col = ch["col"]
            new_val = ch["newVal"]
            orig_row = ch.get("origRow", [])
            columns = ch.get("columns", [])
            set_clause = f"{_safe_ident(col, db_type)} = {_sql_value(new_val, db_type)}"
            # 构建 WHERE：优先主键/唯一索引列，否则所有列
            if where_cols:
            ...
            for cname in target_cols:
                try:
                    ...
                    pass  # column not in the row data, skip
    ...
    try:
        ...
        with engine.begin() as c:
            for ch in changes:
                ...
                columns = ch.get("columns", [])
                set_clause = f"{_safe_ident(col, db_type)} = {_sql_value(new_val, db_type)}"
                target_cols = where_cols if where_cols else columns
                where_parts = []
                for cname in target_cols:
                    try:
                        idx = columns.index(cname)
                ...
                c.execute(text(f"UPDATE {tbl} SET {set_clause} WHERE {where_clause}"))
        engine.dispose()
...
def table_truncate(conn_data, database, table_name, schema=''):
    try:
        ...
        tbl = _build_table_ref(cdata, database, table_name, schema)
        engine = create_engine(_conn_url(cdata), connect_args=_connect_args(cdata.get("db_type","mysql"), timeout=10))
        with engine.begin() as conn: conn.execute(text(f"TRUNCATE TABLE {tbl}"))
        engine.dispose()
...
def table_delete(conn_data, database, table_name, schema=''):
    try:
        ...
        tbl = _build_table_ref(cdata, database, table_name, schema)
        engine = create_engine(_conn_url(cdata), connect_args=_connect_args(cdata.get("db_type","mysql"), timeout=10))
        with engine.begin() as conn: conn.execute(text(f"DROP TABLE {tbl}"))
        engine.dispose()
...
def table_clear(conn_data, database, table_name, schema=''):
    try:
        ...
        tbl = _build_table_ref(cdata, database, table_name, schema)
        engine = create_engine(_conn_url(cdata), connect_args=_connect_args(cdata.get("db_type","mysql"), timeout=10))
        with engine.begin() as conn: conn.execute(text(f"DELETE FROM {tbl}"))
        engine.dispose()
...
_STARTUP_LOG = os.path.join(BASE_DIR, "_startup_diag.txt")

def _diag_log(msg):
    """启动诊断日志（写入文件，因为 console=False 看不到 print）"""
    try:
...
_diag_log(f"BASE_DIR={BASE_DIR}")
_diag_log(f"TREE_FILE={TREE_FILE}")
...
try:
    if not os.path.exists(TREE_FILE):
        _diag_log("INIT: TREE_FILE 不存在，尝试恢复...")
        recovered = _recover_from_backup()
        if not recovered:
            _diag_log("INIT: 无备份可用，创建空文件")
            with open(TREE_FILE, "w", encoding="utf-8") as f:
                json.dump({"folders": [], "connections": {}, "saved_queries": []}, f, ensure_ascii=False, indent=2)
        else:
            _diag_log("INIT: 从备份恢复成功 ({} 连接)".format(len(recovered.get("connections", {}))))
    else:
        file_size = os.path.getsize(TREE_FILE)
        _diag_log(f"INIT: TREE_FILE 存在，大小={file_size} bytes")
        # 文件存在但内容为空或只有空壳数据 → 尝试恢复
        try:
            if file_size < 200:
                _diag_log("INIT: 文件<200字节，检查是否空壳...")
                with open(TREE_FILE, "r", encoding="utf-8") as f:
                    init_data = json.load(f)
                if _is_empty_shell(init_data):
                    _diag_log("INIT: 确认是空壳，尝试恢复...")
                    recovered = _recover_from_backup()
                    if recovered:
                        _diag_log("INIT: 从备份恢复成功 ({} 连接)".format(len(recovered.get("connections", {}))))
        except Exception as e:
            _diag_log(f"INIT: 检查空壳异常: {e}")
except Exception as e:
    _diag_log(f"INIT: 初始化异常: {e}")
...
def _load_tree():
    try:
        ...
        if not content.strip():
            # 文件为空，尝试恢复
            _diag_log("_load_tree: 文件为空")
            recovered = _recover_from_backup()
            if recovered:
                _diag_log(f"_load_tree: 恢复 {len(recovered.get('connections',{}))} 连接")
                return recovered
            _diag_log("_load_tree: 无备份，返回空数据")
            return {"folders": [], "connections": {}, "saved_queries": []}
        data = json.loads(content)
        _diag_log(f"_load_tree: JSON 解析成功, conns={len(data.get('connections',{}))}, folders={len(data.get('folders',[]))}")
        if not _validate_tree(data):
            # 数据格式不正确，尝试恢复
            _diag_log("_load_tree: 结构校验失败")
            recovered = _recover_from_backup()
            if recovered:
                return recovered
            return {"folders": [], "connections": {}, "saved_queries": []}
        ...
        if _is_empty_shell(data):
            _diag_log("_load_tree: 空壳数据，尝试恢复...")
            recovered = _recover_from_backup()
            if recovered:
                _diag_log(f"_load_tree: 恢复 {len(recovered.get('connections',{}))} 连接")
                return recovered
            _diag_log("_load_tree: 空壳+无备份，返回空数据")
        # 正常加载，顺便做一次备份（如果距上次备份超过1小时）
        _maybe_auto_backup()
        _diag_log(f"_load_tree: 正常返回 {len(data.get('connections',{}))} 连接")
        return data
    except json.JSONDecodeError:
        _diag_log("_load_tree: JSON 解析失败")
        recovered = _recover_from_backup()
        if recovered:
            return recovered
        return {"folders": [], "connections": {}, "saved_queries": []}
    except Exception as e:
        _diag_log(f"_load_tree: 异常 {e}")
        return {"folders": [], "connections": {}, "saved_queries": []}
...
def ping():
    ...
    _diag_log("ping() 被调用！WebSocket 通信正常")
    return "pong"

@eel.expose
def tree_load():
    data = _load_tree()
    conn_count = len(data.get("connections", {})) if isinstance(data, dict) else 0
    _diag_log(f"tree_load() 被调用 → 返回 {conn_count} 个连接")
    return data
@eel.expose
            ...
            if file_type == "sql":
                ...
                with engine.begin() as conn:
                    ...
                    for row in rows:
                        ...
                        processed += 1
                        if len(batch) >= batch_size:
                            conn.execute(text(f"INSERT INTO {tbl} ({cols}) VALUES ({ph})"), batch)
                            batch = []
...
if __name__ == "__main__":
    ...
    if getattr(sys, 'frozen', False):
        ...
        if not hasattr(eel, '_eel_js') or not eel._eel_js or len(eel._eel_js) < 100:
            _diag_log("[main] eel._eel_js 缺失，尝试手动加载...")
            ...
            if os.path.exists(eel_js_fallback):
                with open(eel_js_fallback, "r", encoding="utf-8") as f:
                    eel._eel_js = f.read()
                _diag_log(f"[main] 手动加载成功: {len(eel._eel_js)} bytes")
            elif os.path.exists(os.path.join(web_dir, "eel.js")):
                with open(os.path.join(web_dir, "eel.js"), "r", encoding="utf-8") as f:
                    eel._eel_js = f.read()
                _diag_log(f"[main] 从 web/eel.js 加载: {len(eel._eel_js)} bytes")
    else:
        web_dir = "web"
    _diag_log(f"[main] web_dir={web_dir}")
    _diag_log(f"[main] eel_js_exists={os.path.exists(os.path.join(web_dir, 'eel.js'))}")
    _diag_log(f"[main] eel_js_alt_exists={os.path.exists(os.path.join(web_dir, 'js', 'eel.js')) if os.path.exists(os.path.join(web_dir, 'js')) else False}")
    _diag_log(f"[main] _eel_js loaded bytes={len(eel._eel_js) if hasattr(eel, '_eel_js') else 'MISSING'}")
    eel.init(web_dir)
    _diag_log(f"[main] eel.init 完成, exposed_funcs={list(eel._exposed_functions.keys())[:10]}...")
    _diag_log("[main] 启动服务器...")
    try:
        eel.start("index.html", size=(1280, 860), port=0, cmdline_args=['--disable-dev-tools'])
    finally:
        _diag_log("[main] 窗口关闭，退出")
