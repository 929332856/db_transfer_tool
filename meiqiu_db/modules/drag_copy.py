...
def _get_column_info(conn, db_type, db_name, table_name):
    ...
    if db_type in ('mysql', 'ob-mysql'):
        ...
        return [{"name": r[0], "type": r[7] or r[1], "nullable": r[5] == 'YES', "default": r[6],
                 "auto_increment": "auto_increment" in (r[8] or '').lower(), "comment": r[9] or ''} for r in rows]
...
def _generate_create_table(db_type, tbl, cols):
    ...
    for c in cols:
        ...
        if has_default and str(default_raw).strip() == '':
            default_raw = "''"
        dflt = f' DEFAULT {default_raw}' if has_default else ''
        auto = ''
        if c.get('auto_increment'):
            if db_type in ('mysql', 'ob-mysql'):
                auto = ' AUTO_INCREMENT'
            elif db_type in ('postgresql', 'oracle'):
                auto = ' GENERATED BY DEFAULT AS IDENTITY'
            elif db_type == 'mssql':
                auto = ' IDENTITY(1,1)'
        comment = ''
        if c.get('comment') and db_type in ('mysql', 'ob-mysql'):
            comment = " COMMENT '" + str(c['comment']).replace('\\', '\\\\').replace("'", "\\'") + "'"
        col_lines.append(f"  {col_name} {c['type']}{auto}{null}{dflt}{comment}")
